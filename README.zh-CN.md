...
- **图表与图片点击放大**：点击 Mermaid 图或独立 Markdown 图片，在铺满预览区的浮层中查看；可缩放，也可拖动超出视区的图片来平移。带链接的图片保留原有跳转行为。
- **阅读位置直达源码**：选中正文后，点击选区末尾右上方自动出现的铅笔图标；也可以点击标题旁的**铅笔图标**，在同一编辑器组打开对应源码行。
...
| 编辑指定段落 | 选中文本后点击选区末尾右上方的铅笔图标，或点击标题旁的铅笔图标 |
| 复制标题链接 | 点击标题旁的 **#** |
...
点击 Mermaid 图或独立 Markdown 图片可打开图片查看浮层。底部控件依次为缩小、当前缩放比例、放大和适应预览区；图片超出视区时可按住拖动来平移。点击右上角关闭按钮、按 Escape 或点击遮罩可关闭浮层。带 Markdown 链接的图片仍会按原有方式跳转。
